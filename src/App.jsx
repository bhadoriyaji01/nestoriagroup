...
import watermarkPdfPageOne from './assets/img/PDF1_P1_IMG2.png';
import watermarkPdfPageSevenOne from './assets/img/PDF1_P7_IMG1.png';
import watermarkPdfPageSevenTwo from './assets/img/PDF1_P7_IMG2.png';
import watermarkPdfPageSevenThree from './assets/img/PDF1_P7_IMG3.png';
import watermarkPdfPageSevenFour from './assets/img/PDF1_P7_IMG4.png';

const sectionWatermarkImages = [
  watermarkPdfPageOne,
  watermarkPdfPageSevenOne,
  watermarkPdfPageSevenTwo,
  watermarkPdfPageSevenThree,
  watermarkPdfPageSevenFour,
];

function WhiteSectionWatermarks() {
  const location = useLocation();

  useEffect(() => {
    const whiteSections = document.querySelectorAll('#main-content section.bg-white');

    whiteSections.forEach((section, index) => {
      const image = sectionWatermarkImages[
        Math.floor(Math.random() * sectionWatermarkImages.length)
      ];
      const side = Math.random() > 0.5 ? 'left' : 'right';

      section.classList.add('has-section-watermark');
      section.style.setProperty('--section-watermark-image', `url("${image}")`);
      section.style.setProperty('--section-watermark-side', side);
      section.style.setProperty('--section-watermark-order', index);
    });

    return () => {
      whiteSections.forEach((section) => {
        section.classList.remove('has-section-watermark');
        section.style.removeProperty('--section-watermark-image');
        section.style.removeProperty('--section-watermark-side');
        section.style.removeProperty('--section-watermark-order');
      });
    };
  }, [location.pathname]);
  ...
  return (
    <ThemeProvider>
      <HelmetProvider>
        <Router>
          <ScrollToTop />
          <div className="relative bg-slate-50 text-slate-900 min-h-screen" role="main">
            ...
            <Seo 
              ...
              description="Nestoria Group is the premier real estate developer in Dholera SIR offering 100% clear title NA residential, commercial, and industrial investment plots near the Tata Semiconductor Fab."
          ...
          <WhiteSectionWatermarks />
          <Routes>
            <Route path="/" element={<Suspense fallback={<div className="flex justify-center items-center h-64" role="status" aria-label="Loading home page"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500"></div></div>}><><Seo title="Nestoria Group | Top Real Estate Developer in Dholera SIR | Investment Plots" description="Invest in Dholera SIR with Nestoria Group. 100% clear title residential plots, luxury villas & commercial land near Tata Semiconductor Fab & Expressway." keywords="Dholera SIR plots, Dholera Smart City investment, real estate developer in Dholera SIR, buy plots in Dholera, Dholera residential plots, NA title clear plots Dholera, Tata Semiconductor Dholera plots, Ahmedabad Dholera Expressway plots, Dholera airport plots, Nestoria Group" canonicalUrl="https://nestoriagroup.com/" schemaMarkup={[organizationSchema, getWebSiteSchema()]} /><Home /></></Suspense>} />
            ...
            <Route path="/events-gallery" element={<Suspense fallback={<div className="flex justify-center items-center h-64" role="status" aria-label="Loading events gallery page"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500"></div></div>}><><Seo title="Events & Functions Gallery | Nestoria Group Dholera SIR" description="Explore Nestoria Group's event and function gallery featuring investor conclaves, bhoomi pujan ceremonies, festive celebrations, and VIP delegation meets." keywords="Dholera SIR events gallery, Nestoria Group investor conclave, Dholera bhoomi pujan, corporate functions gallery, VIP meetings Dholera" canonicalUrl="https://nestoriagroup.com/events-gallery" schemaMarkup={getBreadcrumbSchema([{ name: 'Home', url: '/' }, { name: 'Events & Functions Gallery', url: '/events-gallery' }])} /><EventsGallery /></></Suspense>} />
